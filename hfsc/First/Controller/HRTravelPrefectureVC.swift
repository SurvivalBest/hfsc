...
class HRTravelPrefectureVC: HRBaseVC ,UITableViewDelegate,UITableViewDataSource,HRFilterViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var cell:HRTravelPreTVCell?
        cell = tableView.dequeueReusableCell(withIdentifier: "CELL1") as? HRTravelPreTVCell
        if cell == nil {
            cell = HRTravelPreTVCell(style: .default, reuseIdentifier: "CELL1")
        }
    ...
    func refreshInfo(type: Int, index: Int) {
        ...
        if type == 0 {
            //分类
            if tourTypeList.count > index {
                self.curType = tourTypeList[index].id
                self.curPage = 1
                self.getData()
            }
        }else{
