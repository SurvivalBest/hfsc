...
class HRShopPrefectureVC: HRBaseVC ,UITableViewDelegate,UITableViewDataSource,HRFilterViewDelegate {
    public var type = 0
    private var curSort = 0
    private var curType = "1"
    private var curPage = 1
    var infoList:[HRShopInfoModel] = []
    ...
    lazy private var headerView:HRFilterView = {
        ...
        tempView.type = 4
        tempView.setFilterCount(titleArr: ["全部","距离最近"])
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        if type == 0 {
            self.setNavTitle(title: "异业联盟")
        }else {
            self.setNavTitle(title: "公司网点")
        }
        self.setUI()
        self.getData()
    ...
    func getData(){
        ...
        HRNetwork.shared.hr_getData(cmd: cmd, params: param, success: { (result) in
            ...
            for (_,subJson):(String,JSON) in result["body"]["list"] {
                let model = HRShopInfoModel.deserialize(from: subJson.rawString()!)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if self.infoList.count > indexPath.row {
            ...
            VC.shopID = self.infoList[indexPath.row].id
    ...
    func refreshInfo(type: Int, index: Int) {
        ...
        if type == 0 {
            ...
            if goodsTypeList.count > index {
                self.curType = goodsTypeList[index].id
                self.curPage = 1
                self.getData()
            }
        }else{
