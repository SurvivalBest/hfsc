...
class HRBindPhoneVC: HRBaseVC,UITextFieldDelegate {
    ...
    func setUI(){
        ...
        self.pwdTF.addSubview(self.getCodeBtn)
        ...
        self.phoneTF.snp.makeConstraints { (make) in
            make.top.equalTo(self.detailLab.snp.bottom).offset(HR_MARGIN)
            make.left.equalTo(HR_MARGIN)
            make.right.equalTo(-HR_MARGIN)
    ...
    func nextStep(){
        ...
        VC.curCode = self.pwdTF.text!
        VC.curType = 1
        VC.platformType = "0"
        self.platformInfo.phone = self.phoneTF.text!
        self.platformInfo.code = self.pwdTF.text!
        VC.platformInfo = self.platformInfo
    ...
    func changeValue(){
        if PhoneNumberIsValidated(vStr: self.phoneTF.text!) && (self.pwdTF.text?.characters.count)! > 3 && self.isSelect {
